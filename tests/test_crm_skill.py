...
def _insert_contact(openkiln_home, **kwargs) -> int:
    ...
    with db.transaction() as conn:
        cursor = conn.execute("INSERT INTO records (type) VALUES ('contact')")
...
def test_list_contacts_not_contacted_since(openkiln_home):
    ...
    _setup(runner, openkiln_home)
    _insert_contact(openkiln_home, email="a@a.com", last_contacted_at=None)
    _insert_contact(openkiln_home, email="b@b.com", last_contacted_at="2099-01-01")
...
def test_tag_contacts_set_segment(openkiln_home):
    ...
    affected = queries.tag_contacts(set_segment="new-seg", filter_segment="old")
    assert affected == 1
...
def test_tag_contacts_remove_tag(openkiln_home):
    ...
    _setup(runner, openkiln_home)
    _insert_contact(openkiln_home, email="a@a.com", tags="keep,remove-me")
...
def test_log_touch_creates_touch_and_updates_contacted_at(openkiln_home):
    ...
    crm_db = openkiln_home / "skills" / "crm.db"
    conn = sqlite3.connect(crm_db)
    touch = conn.execute("SELECT * FROM touches WHERE id = ?", (touch_id,)).fetchone()
    contact = conn.execute(
        "SELECT last_contacted_at FROM contacts WHERE record_id = ?", (record_id,)
    ).fetchone()
    conn.close()
...
def test_get_stats_returns_correct_counts(openkiln_home):
    ...
    segs = {r["segment"]: r["count"] for r in stats["contacts"]["by_segment"]}
    assert segs["seg1"] == 2
    assert segs["seg2"] == 1
...
def test_crm_list_contacts_with_segment_filter(openkiln_home):
    ...
    _setup(runner, openkiln_home)
    _insert_contact(openkiln_home, email="a@a.com", segment="cold-email")
    result = runner.invoke(app, ["crm", "list", "contacts", "--segment", "cold-email"])
    assert result.exit_code == 0
...
def test_crm_tag_dry_run(openkiln_home):
    ...
    _insert_contact(openkiln_home, email="a@a.com", segment="old")
    runner.invoke(
        app, ["crm", "tag", "contacts", "--segment", "old", "--set-segment", "new", "--dry-run"]
    )
...
def test_crm_tag_apply(openkiln_home):
    ...
    result = runner.invoke(
        app, ["crm", "tag", "contacts", "--segment", "old", "--set-segment", "new", "--apply"]
    )
    assert result.exit_code == 0
...
def test_crm_touch_log_command(openkiln_home):
    ...
    record_id = _insert_contact(openkiln_home, email="a@a.com")
    result = runner.invoke(
        app, ["crm", "touch", "log", "--record-id", str(record_id), "--channel", "email"]
    )
    assert result.exit_code == 0
...
def test_crm_list_contacts_json(openkiln_home):
    ...
    result = runner.invoke(app, ["crm", "list", "contacts", "--json"])
    assert result.exit_code == 0
...
def test_tag_contacts_missing_action_gives_clear_error(openkiln_home):
    ...
    _setup(runner, openkiln_home)
    result = runner.invoke(app, ["crm", "tag", "contacts", "--segment", "something"])
...
def test_crm_reset_dry_run(openkiln_home):
    ...
    _setup(runner, openkiln_home)
    _insert_contact(openkiln_home, email="a@a.com")
    result = runner.invoke(app, ["crm", "reset", "contacts", "--dry-run"])
    assert result.exit_code == 0
...
def test_crm_reset_apply_deletes_contacts(openkiln_home):
    ...
    result = runner.invoke(app, ["crm", "reset", "contacts", "--apply"])
    assert result.exit_code == 0
    rows = queries.list_contacts()
    assert len(rows) == 0
...
def test_crm_reset_unknown_entity_fails(openkiln_home):
    ...
    _setup(runner, openkiln_home)
    result = runner.invoke(app, ["crm", "reset", "invoices", "--apply"])
    assert result.exit_code == 1
...
def _insert_company(openkiln_home, **kwargs) -> int:
    ...
    with db.transaction() as conn:
        cursor = conn.execute("INSERT INTO records (type) VALUES ('company')")
        record_id = cursor.lastrowid
    ...
    cols = ", ".join(fields.keys())
    placeholders = ", ".join(["?"] * len(fields))
    conn.execute(f"INSERT INTO companies ({cols}) VALUES ({placeholders})", list(fields.values()))
    conn.commit()
    conn.close()
    return record_id
...
def test_link_contacts_by_email_domain(openkiln_home):
    ...
    company_id = _insert_company(openkiln_home, name="Acme Corp", domain="acme.com")
    ...
    conn = sqlite3.connect(crm_db)
    row = conn.execute(
        "SELECT company_record_id FROM contacts WHERE record_id = ?", (contact_id,)
    ).fetchone()
    conn.close()
...
def test_link_contacts_dry_run_does_not_write(openkiln_home):
    ...
    crm_db = openkiln_home / "skills" / "crm.db"
    conn = sqlite3.connect(crm_db)
    row = conn.execute("SELECT company_record_id FROM contacts LIMIT 1").fetchone()
    conn.close()
...
def test_link_contacts_skips_already_linked(openkiln_home):
    ...
    _setup(runner, openkiln_home)
    company_id = _insert_company(openkiln_home, domain="acme.com")
    _insert_contact(openkiln_home, email="john@acme.com", company_record_id=company_id)
...
def test_link_contacts_overwrite(openkiln_home):
    ...
    _setup(runner, openkiln_home)
    old_company_id = _insert_company(openkiln_home, domain="old.com")
    new_company_id = _insert_company(openkiln_home, domain="acme.com")
    contact_id = _insert_contact(openkiln_home, email="john@acme.com", company_record_id=old_company_id)
...
def test_crm_link_contacts_command_dry_run(openkiln_home):
    ...
    result = runner.invoke(app, ["crm", "link", "contacts", "--dry-run"])
    assert result.exit_code == 0
...
def test_crm_link_contacts_command_apply(openkiln_home):
    ...
    result = runner.invoke(app, ["crm", "link", "contacts", "--apply"])
    assert result.exit_code == 0
...
def test_crm_link_contact_manual_command(openkiln_home):
    ...
    result = runner.invoke(
        app, ["crm", "link", "contact", "--contact-id", str(contact_id), "--company-id", str(company_id)]
    )
    assert result.exit_code == 0
...
def test_crm_lists_create_command(openkiln_home):
    ...
    _setup(runner, openkiln_home)
    result = runner.invoke(app, ["crm", "lists", "create", "my-list"])
    assert result.exit_code == 0
...
def test_crm_lists_add_and_members(openkiln_home):
    ...
    result = runner.invoke(app, ["crm", "lists", "add", "test-list", "--ids", str(rid)])
    assert result.exit_code == 0

    result2 = runner.invoke(app, ["crm", "lists", "members", "test-list"])
...
def test_crm_lists_delete_command(openkiln_home):
    ...
    result = runner.invoke(app, ["crm", "lists", "delete", "to-delete"])
    assert result.exit_code == 0
    assert len(queries.get_lists()) == 0
