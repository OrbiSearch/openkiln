...
def test_inspect_with_skill_shows_mapping(openkiln_home, tmp_path):
    ...
    result = runner.invoke(app, ["record", "inspect", str(f), "--skill", "crm"])
    assert result.exit_code == 0
    assert "email" in result.output
...
def test_inspect_json(openkiln_home, tmp_path):
    ...
    result = runner.invoke(app, ["record", "inspect", str(f), "--json"])
    assert result.exit_code == 0
...
def test_import_dry_run_touches_nothing(openkiln_home, tmp_path):
    ...
    runner.invoke(
        app,
        ["record", "import", str(f), "--type", "contact", "--skill", "crm", "--dry-run"],
    )
...
def test_import_apply_creates_records(openkiln_home, tmp_path):
    ...
    result = runner.invoke(
        app,
        ["record", "import", str(f), "--type", "contact", "--skill", "crm", "--apply"],
    )
    assert result.exit_code == 0
    with db.connection() as conn:
        count = conn.execute("SELECT COUNT(*) FROM records WHERE type = 'contact'").fetchone()[0]
    assert count == 2
...
def test_import_apply_creates_skill_records(openkiln_home, tmp_path):
    ...
    row = conn.execute(
        "SELECT first_name, last_name FROM contacts WHERE email = ?", ("john@acme.com",)
    ).fetchone()
    conn.close()
...
def test_import_without_skill_creates_bare_records(openkiln_home, tmp_path):
    ...
    result = runner.invoke(
        app,
        ["record", "import", str(f), "--type", "contact", "--apply"],
    )
    assert result.exit_code == 0
    with db.connection() as conn:
        count = conn.execute("SELECT COUNT(*) FROM records WHERE type = 'contact'").fetchone()[0]
...
def test_import_json_output(openkiln_home, tmp_path):
    ...
    result = runner.invoke(
        app,
        ["record", "import", str(f), "--type", "contact", "--skill", "crm", "--apply", "--json"],
    )
    assert result.exit_code == 0
    data = json.loads(result.output)
...
def test_import_map_remaps_columns(openkiln_home, tmp_path):
    ...
    f = _make_csv(
        tmp_path,
        [
            {"Title": "CEO", "EmailAddress": "john@acme.com", "FullName": "John Smith"},
        ],
    )
    result = runner.invoke(
        app,
    ...
    row = conn.execute("SELECT job_title, email, full_name FROM contacts LIMIT 1").fetchone()
    conn.close()
    assert row[0] == "CEO"
...
def test_import_dry_run_suggests_map_for_skipped_columns(openkiln_home, tmp_path):
    """Dry-run output suggests --map for skipped columns."""
    _setup(runner, openkiln_home)
    f = _make_csv(tmp_path, [{"email": "a@b.com", "mystery_col": "value"}])
    result = runner.invoke(
        app,
        ["record", "import", str(f), "--type", "contact", "--skill", "crm", "--dry-run"],
    )
...
def test_import_upsert_updates_existing_records(openkiln_home, tmp_path):
    ...
    f2 = _make_csv(
        tmp_path,
        [
            {"email": "john@acme.com", "first_name": "Jonathan", "job_title": "CEO"},
        ],
    )
    ...
    with open(f2, "w", newline="") as fh:
        writer = csv_mod.DictWriter(fh, fieldnames=["email", "first_name", "job_title"])
        writer.writeheader()
        writer.writerow(
            {
                "email": "john@acme.com",
    ...
    result = runner.invoke(
        app,
        ["record", "import", str(f2), "--type", "contact", "--skill", "crm", "--upsert", "--apply"],
    )
    assert result.exit_code == 0
    ...
    conn = sqlite3.connect(openkiln_home / "skills" / "crm.db")
    row = conn.execute(
        "SELECT first_name, job_title FROM contacts WHERE email = 'john@acme.com'"
    ).fetchone()
    conn.close()
...
def test_import_map_dedup_uses_mapped_column(openkiln_home, tmp_path):
    ...
    conn = sqlite3.connect(openkiln_home / "skills" / "crm.db")
    count = conn.execute("SELECT COUNT(*) FROM companies WHERE domain = 'acme.com'").fetchone()[0]
    conn.close()
...
def test_domain_normalised_on_import(openkiln_home, tmp_path):
    ...
    f = _make_csv(
        tmp_path,
        [
            {"domain": "https://www.acme.com/about", "name": "Acme Corp"},
            {"domain": "http://corp.io/", "name": "Corp"},
            {"domain": "www.example.com", "name": "Example"},
            {"domain": "clean.com", "name": "Clean"},
        ],
    )
    runner.invoke(
        app,
        ["record", "import", str(f), "--type", "company", "--skill", "crm", "--apply"],
    )

    import sqlite3

    conn = sqlite3.connect(openkiln_home / "skills" / "crm.db")
    domains = {row[0] for row in conn.execute("SELECT domain FROM companies").fetchall()}
    conn.close()
